...
import modules.requests_content_Calidra_Zapotlan
import modules.requests_content_Calidra_Cienega
import modules.requests_content_Sandos
    ...
    with cnxn:
        ...
        while current_date <= final_date:
            query = """
                    SELECT * 
                    FROM [dbo].[ODC Beetmann] 
                    WHERE [Fecha de la oferta de compra] = '{date}' and Cliente = 'Artigraf'
                    """.format(date=current_date)
            existing_ODC = pd.read_sql(query, cnxn)
...
Convert(DATE, (([Fecha]+[Hora])-(.003472222))) AS 'Fecha', 
((DATEPART(hour,(([Fecha]+[Hora])-(.003472222))))+1) AS 'Hora', 
(ROUND(((sum([Kwhe]))/1000),4)*1.108647450110865000) * .2 AS 'Valor' 
    ...
    with cnxn:
        ...
        while current_date <= final_date:
            query = """
                    SELECT * 
                    FROM [dbo].[ODC Beetmann] 
                    WHERE [Fecha de la oferta de compra] = '{date}' and Cliente = 'CALIDRA ZAPOTLAN'
                    """.format(date=current_date)
            existing_ODC = pd.read_sql(query, cnxn)
...
((DATEPART(hour,(([Fecha]+[Hora])-(.003472222))))+1) AS 'Hora', 
(ROUND(((sum([Kwhe]))/1000),4)*1.124859392575928)  AS 'Valor' 
FROM [dbo].[Cincominutal CALIDRA] 
WHERE (Convert(DATE, (([Fecha]+[Hora])-(.003472222))) = '{d_1}' OR Convert(DATE, (([Fecha]+[Hora])-(.003472222))) = '{d_2}'
OR Convert(DATE, (([Fecha]+[Hora])-(.003472222))) = '{d_3}' OR Convert(DATE, (([Fecha]+[Hora])-(.003472222))) = '{d_4}') and
(equipo = '460081001244' or equipo = '460950300460')
GROUP BY  Convert(DATE, (([Fecha]+[Hora])-(.003472222))), ((DATEPART(hour,(([Fecha]+[Hora])-(.003472222))))+1)
) 
AS table_1 
GROUP BY table_1.Hora
                ...
                if df.empty:
                    ...
                    url = 'https://ws01.cenace.gob.mx:8082/mxswmem/EnviarOfertaCompraEnergiaService.asmx?wsdl'
                    headers = {'content-type': 'text/xml'}
                    body = modules.requests_content_Calidra_Zapotlan.body
    ...
    with cnxn:
        ...
        while current_date <= final_date:
            query = """
                    SELECT * 
                    FROM [dbo].[ODC Beetmann] 
                    WHERE [Fecha de la oferta de compra] = '{date}' and Cliente = 'CALIDRA CIENEGA'
                    """.format(date=current_date)
            existing_ODC = pd.read_sql(query, cnxn)
...
GROUP BY table_1.Hora
                ...
                if df.empty:
                    ...
                    url = 'https://ws01.cenace.gob.mx:8082/mxswmem/EnviarOfertaCompraEnergiaService.asmx?wsdl'
                    headers = {'content-type': 'text/xml'}
                    body = modules.requests_content_Calidra_Cienega.body
    ...
    with cnxn:
        ...
        while current_date <= final_date:
            query = """
                    SELECT * 
                    FROM [dbo].[ODC Beetmann] 
                    WHERE [Fecha de la oferta de compra] = '{date}' and Cliente = 'Sandos'
                    """.format(date=current_date)
            existing_ODC = pd.read_sql(query, cnxn)
...
((DATEPART(hour,(([Fecha]+[Hora])-(.003472222))))+1) AS 'Hora', 
(ROUND(((sum([Kwhe]))/1000),4)*1.108647450110865000) * .2 AS 'Valor' 
FROM [dbo].[Cincominutales_medimem] 
WHERE (Convert(DATE, (([Fecha]+[Hora])-(.003472222))) = '{d_1}' OR Convert(DATE, (([Fecha]+[Hora])-(.003472222))) = '{d_2}'
OR Convert(DATE, (([Fecha]+[Hora])-(.003472222))) = '{d_3}' OR Convert(DATE, (([Fecha]+[Hora])-(.003472222))) = '{d_4}') and
(equipo = 'DW22A129' )
GROUP BY  Convert(DATE, (([Fecha]+[Hora])-(.003472222))), ((DATEPART(hour,(([Fecha]+[Hora])-(.003472222))))+1)
...
GROUP BY table_1.Hora
                ...
                if df.empty:
                    ...
                    url = 'https://ws01.cenace.gob.mx:8082/mxswmem/EnviarOfertaCompraEnergiaService.asmx?wsdl'
                    headers = {'content-type': 'text/xml'}
                    body = modules.requests_content_Sandos.body
...
if __name__ == '__main__':
    ...
    print("iniciando envio de ODC para Calidra Zapotlan")
    main_CALIDRA_ZAPOTLAN()

    print("iniciando envio de ODC para Calidra Cienega")
    main_CALIDRA_Cienega()
    
    print("iniciando envio de ODC para SANDOS")
    main_Sandos()